...
public class PromptIntelligenceEngineDegradationTests
{
    ...
    [Fact]
    public async Task ProcessAsync_OptimizationFails_ConstraintsPreserved()
    {
        _mockOptimizer.Setup(o => o.Optimize(It.IsAny<string>()))
            .Throws(new Exception("Optimization error"));
        ...
        Assert.True(result.ConstraintsPreserved);
    }
    ...
    [Fact]
    public async Task ProcessAsync_NullRequest_ReturnsFailed()
    {
        var result = await _engine.ProcessAsync(null!, "user1");
