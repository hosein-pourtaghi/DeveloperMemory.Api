...
public class KeywordRetrievalProviderTests : IDisposable
{
    ...
    [Fact]
    public async Task CrossWorkspaceIsolation_FullScenario()
    {
        var workspaceAMemory = TestDataHelper.CreateMemory(
            title: "Workspace A Secret", scope: MemoryScope.Workspace, workspaceId: "ws-A");
        var workspaceBMemory = TestDataHelper.CreateMemory(
            title: "Workspace B Secret", scope: MemoryScope.Workspace, workspaceId: "ws-B");
        var globalMemory = TestDataHelper.CreateMemory(
            title: "Shared Knowledge", scope: MemoryScope.Global);

        _fixture.Context.MemoryEntries.AddRange(workspaceAMemory, workspaceBMemory, globalMemory);
        await _fixture.Context.SaveChangesAsync();

        var requestA = TestDataHelper.CreateRetrievalRequest(
            query: "Secret", workspaceId: "ws-A");
        var resultsA = await _provider.GetCandidatesAsync(requestA);

        resultsA.Should().Contain(m => m.Id == workspaceAMemory.Id,
            "Workspace A memory should be returned for Workspace A");
        resultsA.Should().NotContain(m => m.Id == workspaceBMemory.Id,
            "Workspace B memory must NOT leak to Workspace A");
        resultsA.Should().Contain(m => m.Id == globalMemory.Id,
            "Global memory should be available to Workspace A");
    }
    ...
    [Fact]
    public async Task CrossUserIsolation_FullScenario()
    {
        var userAMemory = TestDataHelper.CreateMemory(
            title: "User A Secret", scope: MemoryScope.Private, userId: "user-A");
        var userBMemory = TestDataHelper.CreateMemory(
            title: "User B Secret", scope: MemoryScope.Private, userId: "user-B");
        var globalMemory = TestDataHelper.CreateMemory(
            title: "Shared Knowledge", scope: MemoryScope.Global);

        _fixture.Context.MemoryEntries.AddRange(userAMemory, userBMemory, globalMemory);
        await _fixture.Context.SaveChangesAsync();

        var requestA = TestDataHelper.CreateRetrievalRequest(
            query: "Secret", userId: "user-A");
        var resultsA = await _provider.GetCandidatesAsync(requestA);

        resultsA.Should().Contain(m => m.Id == userAMemory.Id,
            "User A memory should be returned for User A");
        resultsA.Should().NotContain(m => m.Id == userBMemory.Id,
            "User B memory must NOT leak to User A");
        resultsA.Should().Contain(m => m.Id == globalMemory.Id,
            "Global memory should be available to User A");
    }
    ...
    [Fact]
    public async Task RetrieveAsync_CrossProjectIsolation_FullScenario()
    {
        var projectA = Guid.NewGuid();
        var projectB = Guid.NewGuid();

        var projectAMemory = TestDataHelper.CreateMemory(
            title: "Project A Secret", scope: MemoryScope.Project, projectId: projectA);
        var projectBMemory = TestDataHelper.CreateMemory(
            title: "Project B Secret", scope: MemoryScope.Project, projectId: projectB);
        var globalMemory = TestDataHelper.CreateMemory(
            title: "Shared Knowledge", scope: MemoryScope.Global);
        var privateMemory = TestDataHelper.CreateMemory(
            title: "Private Notes", scope: MemoryScope.Private, userId: "user-1");
        ...
        _fixture.Context.MemoryEntries.AddRange(
            projectAMemory, projectBMemory, globalMemory, privateMemory);
        await _fixture.Context.SaveChangesAsync();

        var requestA = TestDataHelper.CreateRetrievalRequest(
            query: "Secret", projectId: projectA, userId: "user-1");
        var resultsA = await _provider.GetCandidatesAsync(requestA);

        resultsA.Should().Contain(m => m.Id == projectAMemory.Id,
            "Project A memory should be returned for Project A");
        resultsA.Should().NotContain(m => m.Id == projectBMemory.Id,
            "Project B memory must NOT leak to Project A");
        resultsA.Should().Contain(m => m.Id == globalMemory.Id,
            "Global memory should be available to Project A");
        resultsA.Should().Contain(m => m.Id == privateMemory.Id,
            "Private memory should be available to the user");
    }
